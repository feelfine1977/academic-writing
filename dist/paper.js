...
export function createPaperUI({api,esc,heading,list,toast,modal,teachingUI,getState,isCurrent,refresh}) {
 ...
 function catalogue(){
  ...
  const advanced=s.learning?.courses.filter(c=>c.kind==='advanced')||[];
  ...
  $('#main').innerHTML=heading('Practise the English your research needs.','Choose a difficulty, a research topic and the language skill you want to learn.', '<a class="btn" href="#papers">Open Papers</a>')+`
   ${advanced.length?`<section class="practice-discovery"><p><strong>Ready for a challenge?</strong> ${advanced.reduce((n,c)=>n+c.total,0)} advanced activities across ${advanced.length} courses.</p><a class="btn secondary" href="#courses/advanced">Explore advanced English →</a></section>`:''}<section class="card"><div class="catalogue-filters">
