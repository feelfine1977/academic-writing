...
const labels={papers:'My papers',revision:'WISE revision workspace',obsidian:'Obsidian & iPad',today:'Today',courses:'Courses & progress',paper:'My WISE paper',practice:'Skill practice',reader:'WISE reader reviews',comments:'My comments',writing:'My writing',library:'Research library',progress:'Learning history',guide:'How to use the lab',phrasebook:'Academic phrasebook',settings:'Settings & backups'};
...
function heading(title,sub,extra=''){return `<div class="page-heading"><div><div class="eyebrow">Your academic writing practice</div><h1>${esc(title)}</h1><p class="subtitle">${esc(sub)}</p></div>${extra}</div>`}
...
async function refresh(){state=await api('/bootstrap');const wiseLink=document.querySelector('[data-page="paper"]');if(wiseLink&&state.workspace?.legacy_paper_id)wiseLink.href='#papers/'+encodeURIComponent(state.workspace.legacy_paper_id);$('#tutor-status').textContent=state.profile.model?`Local tutor · ${state.profile.model}`:'Tutor · choose a local model';}
...
async function practice(id,serial,from,requestedSession,requestedJob){
 ...
 const e=state.exercises.find(x=>x.key===id||x.id===id)||state.exercises[0];
 if(!e){$('#main').innerHTML=heading('Your practice starts here.','Create an exercise pack in the research library.');return}
 ...
 if(session.exercise_id!==e.key)throw new Error('This saved session belongs to a different exercise. Open it from My writing.');
 ...
 active={e,session:full,cache,closed,saving:false,requestKey:null,returnNode:from&&state.exercises.some(x=>x.paper_node_id===from)?from:null};
 activityUI.remember(e,full,active.returnNode);
 if(!active.returnNode)localPut('awl-practice-resume',e.key);
 $('#main').innerHTML=heading('A skill worth making your own.','Write → submit → use the feedback → revise in your own words.')+`
 <div class="exercise-shell"><aside class="exercise-list" aria-label="Related exercises"><a href="#practice" class="btn secondary">Find exercises</a>${state.exercises.filter(x=>!x.archived_version&&(e.paper_node_id?x.paper_node_id===e.paper_node_id:e.family?x.family===e.family:x.pack_key===e.pack_key)).slice(0,40).map((x,i)=>`<a href="${href(x)}" class="exercise-item ${x.key===e.key?'active':''}"><span>${String(i+1).padStart(2,'0')}</span><span>${esc(x.title)}<small data-exercise-status="${esc(x.key)}">${courseUI.statusLabel(courseUI.status(x.key))}</small></span></a>`).join('')}</aside>
 ...
 ${e.choices?.length?`<div class="pill-row" aria-label="${purposeChoice?'Answer options':'Word bank'}">${e.choices.map((x,i)=>purposeChoice?`<button type="button" class="choice-chip purpose-choice" data-purpose-choice="${i}" aria-pressed="false">${esc(x)}</button>`:`<span class="choice-chip">${esc(x)}</span>`).join('')}</div>`:''}
 ${!closed&&e.criteria?.length?`<details class="criteria-guide"><summary>What this exercise practises</summary>${list(e.criteria)}</details>`:''}
 ...
 <label class="editor-label" for="answer">Your answer</label>
 ${e.format==='clause_completion'?'<p class="tiny">Write just the missing clause or the complete sentence. Both are accepted.</p>':''}
 ${purposeChoice?'<p class="tiny">Click an option, then Check answer. You can also type the option, or write “The paragraph should…” followed by that option.</p>':e.format==='gap'?'<p class="tiny">Enter the missing word(s) only. Separate multiple gaps with /.</p>':''}
 ...
 <div class="submit-bar"><button class="btn" id="submit-answer">${closed?'Check answer':'Save & get feedback'}</button><button class="btn secondary" id="save-only">Save attempt only</button><span class="tiny">${closed?'Enter to check · Enter again after a correct result to continue. Shift + Enter adds a line.':'⌘ / Ctrl + Enter to submit · Enter adds a new line.'}</span></div>
 ...
 <details id="attempt-history"><summary>My saved attempts (<span id="attempt-count">${full.attempts.length}</span>)</summary><div id="history"></div></details>
 ...
 $('#practice-review-mode').onchange=async event=>{const control=event.target,prior=state.profile.review_mode||'careful';control.disabled=true;$('#submit-answer').disabled=true;try{const profile=Object.fromEntries(['provider','model','language','local_confirmed','verifier_model'].filter(k=>state.profile[k]!==undefined).map(k=>[k,state.profile[k]]));state.profile=await api('/settings',{...profile,review_mode:control.value},'PUT');toast('Review depth saved for your next submission.')}catch(error){control.value=prior;toast(error.message)}finally{if(active===context){control.disabled=false;$('#submit-answer').disabled=false}}};
 courseUI.practiceProgress(e);courseUI.completionReview(context,()=>active===context);
 if(e.workspace_paper_id){
  const back='#papers/'+encodeURIComponent(e.workspace_paper_id)+'?card='+encodeURIComponent(e.workspace_card_id);
  $('.exercise-body').insertAdjacentHTML('afterbegin',`<div class="paper-practice-return"><a href="${back}">← Return to my argument · ${esc(e.workspace_context.argument_title)}</a><p class="tiny">You are reviewing a saved copy. Save a revised attempt here, then explicitly return it to the argument card.</p><button class="btn secondary" id="apply-paper-attempt">Save revised answer to my argument</button></div>`);
  $('#apply-paper-attempt').onclick=async()=>{try{if(context.saving)throw new Error('Wait until the saved attempt is ready.');const attempt=context.session.attempts.at(-1);if(!attempt||attempt.text!==$('#answer').value)throw new Error('Save your revised answer first.');await api(`/workspace/papers/${e.workspace_paper_id}/cards/${e.workspace_card_id}/apply-attempt`,{attempt_id:attempt.id});location.hash=back}catch(error){toast(error.message)}};
 }
...
async function submitAnswer(feedback){
 ...
 try{
   ...
   $('#save-status').textContent=unchanged?`Attempt ${result.attempt_count} saved on this Mac.`:`Attempt ${result.attempt_count} saved; your newer edits are kept in this browser. Save again when ready.`;
   $('#answer-feedback').innerHTML=`<div class="alert ${result.correct?'success':''}"><strong>✓ Attempt ${result.attempt_count} saved.</strong> ${result.checked?esc(result.message):'Open writing has many valid answers. Use the criteria and tutor feedback to revise.'}</div>${result.explanation?`<p>${esc(result.explanation)}</p>`:''}${result.feedback_error?`<div class="alert">${esc(result.feedback_error)} <a href="#settings">Open tutor settings</a></div>`:''}`;
   c.advance=null;$('#next-exercise-area').innerHTML='';
   if(result.checked&&result.correct&&$('#answer').value===text){
...
function sessionsTable(rows){return rows.length?`<div class="table-wrap"><table><thead><tr><th>Exercise</th><th>Attempts</th><th>Last saved</th><th>Note</th></tr></thead><tbody>${rows.map(s=>`<tr><td><a href="#practice/${encodeURIComponent(s.exercise_id)}">${esc(s.title)}</a></td><td>${s.attempts}</td><td>${date(s.updated)}</td><td><a href="/api/exports/${s.id}">Download</a></td></tr>`).join('')}</tbody></table></div>`:'<p class="empty">No saved writing yet. Start an exercise or create a paragraph task.</p>'}
async function writing(serial){
 $('#main').innerHTML=heading('Your writing, over time.','Keep your first attempt, your revisions, and the reason for each change.', '<button class="btn" id="new-writing">Write from a new outline</button>')+`<section class="card">${sessionsTable(state.sessions.filter(s=>s.attempts))}</section>`;
...
async function builder(sources=[],paragraph=false){
 ...
 $('#build-pack').onclick=async()=>{const b=$('#build-pack');b.disabled=true;try{const r=await api('/packs/build',{title:$('#pack-title').value,outline:$('#pack-outline').value,confirmed:$('#confirm-ideas').checked,source_ids:sources.map(s=>s.id),skill:'S03',paragraph_only:paragraph});await refresh();$('#modal').close();location.hash=href(state.exercises.find(e=>e.pack_key===r.pack_id));toast(`${r.count} exercises created. Download their files in Research library.`)}catch(e){$('#builder-status').textContent=e.message}finally{b.disabled=false}};
}
...
async function settings(serial){
 $('#main').innerHTML=heading('A tutor on your own Mac.','Choose a local model, set the feedback language, and keep a backup of your work.')+`
 ...
 $('#save-profile').onclick=async()=>{try{state.profile=await api('/settings',{provider:$('#provider').value,model:$('#model').value,language:$('#language').value,local_confirmed:true,review_mode:$('#review-mode').value,verifier_model:$('#verifier-model').value},'PUT');await refresh();$('#profile-status').textContent='Tutor selected. Open an exercise and choose Save & get feedback.'}catch(e){$('#profile-status').textContent=e.message}};
...
async function render(){
 const serial=++routeSerial;await papersUI.leave();if(serial!==routeSerial)return;active=null;clearTimeout(pollTimer);
 let [page,id]=location.hash.slice(1).split('/');const [rawId,query]=id?.split('?')||[];const params=new URLSearchParams(query||'');const from=params.get('from');page=labels[page]?page:'today';id=rawId?decodeURIComponent(rawId):undefined;
 document.body.classList.toggle('paper-workspace-active',page==='papers');$('#page-label').textContent=labels[page];document.querySelectorAll('nav a').forEach(a=>a.classList.toggle('active',a.dataset.page===page));
 try{if(page==='papers')await papersUI.render(id,serial,params.get('card'));else if(page==='today')await today(serial);else if(page==='revision')await revisionUI.render(id,serial,{document:params.get('document'),segment:params.get('segment'),reuse:params.get('reuse')});else if(page==='guide')renderGuide(heading);else if(page==='obsidian')await obsidianUI.render(id,serial);else if(page==='courses'){await courseUI.render(id,serial)}else if(page==='reader'||page==='comments'||page==='phrasebook'){await teachingUI[page](id,serial)}else if(page==='paper'){ $('#main').innerHTML='<p class="loading">Opening your paper…</p>';await paperUI.render(id,serial)}else{ $('#main').innerHTML='<p class="loading">Opening your workspace…</p>';await ({practice,writing,library,progress,settings}[page])(page==='practice'?id:serial,page==='practice'?serial:undefined,from,params.get('session'),params.get('job'))}}catch(e){if(serial===routeSerial)$('#main').innerHTML=`<div class="alert error">${esc(e.message)} Your saved attempts remain on this Mac. <a href="#today">Return to Today</a></div>`}
