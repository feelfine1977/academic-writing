...
export function createActivityUI({api,esc,toast}) {
 ...
 function remember(e,session,from){
  ...
  pins.unshift({exercise_id:e.key,title:e.title,session_id:session.id,from,updated:Date.now()});
  write('awl-work-pins',pins.slice(0,5));draw();
 }
 function href(p){if(p.kind==='paper')return '#paper/'+encodeURIComponent(p.node_id);const q=new URLSearchParams({session:p.session_id});if(p.id)q.set('job',p.id);if(p.from||p.paper_node_id)q.set('from',p.from||p.paper_node_id);return '#practice/'+encodeURIComponent(p.exercise_id)+'?'+q}
 function draw(){
  ...
  const items=[...pending,...recent.slice(0,4),...pins.filter(p=>p.kind==='paper'||!jobs.some(j=>j.session_id===p.session_id))];
  ...
  const status=p=>p.kind==='paper'?'Return to WISE paragraph':p.preview_ready?'First feedback ready · checking':({running:'Tutor is reading',queued:'Waiting for tutor',complete:'Feedback ready',failed:'Review needs a retry',cancelled:'Review cancelled',interrupted:'Review interrupted · retry'}[p.status]||'Resume exercise');
  ...
  host.innerHTML=`<div class="continue-primary"><strong>Continue your work</strong><a href="${esc(href(items[0]))}">${esc(items[0].title)} <span class="tag">${esc(status(items[0]))}</span></a>${pins.find(p=>p.kind==='paper')?`<a class="continue-wise" href="${esc(href(pins.find(p=>p.kind==='paper')))}">↩ WISE · ${esc(pins.find(p=>p.kind==='paper').node_id)}</a>`:''}</div><details><summary>All pinned work (${items.length})</summary><ul>${items.map(p=>`<li><a href="${esc(href(p))}"><strong>${esc(p.title)}</strong><small>${esc(status(p))}${p.created?' · '+esc(new Date(p.created).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'})):''}</small></a>${!['queued','running'].includes(p.status)?`<button class="btn quiet" data-dismiss-work="${esc(p.id||p.exercise_id)}" aria-label="Unpin ${esc(p.title)}">Unpin</button>`:''}</li>`).join('')}</ul><p class="tiny">Saved answers stay in My writing after unpinning. Tutor reviews continue while you use other pages.</p></details>`;
