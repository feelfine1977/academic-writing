...
export function createPaperSourcesUI({api,esc,modal,toast}){
 ...
 async function upload(paper){
  modal('Import manuscript or supervisor remarks',`<p>Your source is kept separately from your argument plan and editable prose.</p><label class="field">Source file<input type="file" id="paper-source-file" accept=".pdf,.tex,.docx,.md,.txt"></label><label class="field">PDF layout<select id="paper-source-columns"><option value="2">Two columns</option><option value="1">One column</option></select></label><label><input type="checkbox" id="paper-source-black"> In this PDF, black text means wording agreed with my supervisors.</label><p class="tiny">Only select this if it is your document’s colour convention. All colours remain searchable. TeX is imported as text; included files are not loaded. Up to 16 MB.</p><button class="btn" id="paper-source-upload">Import source</button><p role="status" id="paper-source-status"></p>`);
  $('#paper-source-upload').onclick=async()=>{const button=$('#paper-source-upload');button.disabled=true;try{
   const file=$('#paper-source-file').files[0];if(!file)throw new Error('Choose a source file first.');if(file.size>16*1024*1024)throw new Error('Choose a source no larger than 16 MB.');
   $('#paper-source-status').textContent='Extracting passages and saving the source to your vault…';
   ...
   const source=await api(`/workspace/papers/${paper}/sources`,{filename:file.name,base64,columns:Number($('#paper-source-columns').value),black_means_agreed:$('#paper-source-black').checked});
   $('#paper-source-status').textContent=`Imported ${source.segment_count} passages. Close this window and choose Find source passages to search them.`;button.textContent='Imported';
  }catch(e){$('#paper-source-status').textContent=e.message;button.disabled=false}};
 }
