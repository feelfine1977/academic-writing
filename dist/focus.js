...
export function createFocusUI({api,esc,toast}) {
 ...
 const safeRoute=r=>/^#(?:paper|papers|practice|revision|reading|fiction|section-writing)\/[A-Za-z0-9%_:@.?=&-]+$/.test(r||'')?r:'#learning';
 const inProgress=()=>state&&['running','paused','wrapping'].includes(state.phase);
 function showRecent(){const list=$('#focus-recent-list');if(!list)return;list.innerHTML=recent.length?recent.map(p=>`<a href="${esc(safeRoute(p.href))}"><strong>${esc(p.title)}</strong><small>${esc(p.status)}</small></a>`).join(''):'<p>No recent work yet. Open an exercise or a paper argument.</p>';$('#focus-recent-count').textContent=recent.length?` (${recent.length})`:'';}
 function mode(){
 ...
 window.addEventListener('awl-writing-round-start',e=>{const next=e.detail;if(!next||![10,25,45].includes(next.minutes))return;if(inProgress()){toast('Your current timer continues. The new writing goal is saved in your section.');return}task=next;start(next.minutes,next.goal,next.route);});
