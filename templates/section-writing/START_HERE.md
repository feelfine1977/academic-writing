...
## Keep supervisor guidance with the paper

Copy `Tutor knowledge.md` into the paper’s `Section writing` folder when you want the tutor to use checked meeting or feedback records. Keep readable source extracts in `Section writing/Sources`; the template starts with an empty index. Curate only guidance relevant to the current task, retain exact quotations and mark earlier instructions historical or superseded when appropriate. Copying a transcript alone does not activate every remark as a requirement. The app repository’s `docs/tutor-alignment.md` explains the format and includes a fictional example.

Choose the outline idea you are working on before requesting a section review. The tutor’s first reading is provisional; proposed action items receive an additional local-model check. You can keep editing the saved draft while it runs. Source matching and a second check do not establish supervisor approval, and you can keep wording that already works. Private guidance, reviews and writing checkpoints travel with the paper through Obsidian Sync.
